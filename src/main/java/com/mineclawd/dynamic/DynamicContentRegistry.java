...
public final class DynamicContentRegistry {
    ...
    public static void syncToPlayer(ServerPlayerEntity player) {
        ...
        if (!ServerPlayNetworking.canSend(player, MineClawdNetworking.SYNC_DYNAMIC_CONTENT)) {
            return;
        }
        var buf = PacketByteBufs.create();
        buf.writeString(buildSyncPayload());
        ServerPlayNetworking.send(player, MineClawdNetworking.SYNC_DYNAMIC_CONTENT, buf);
    }
    ...
    public static synchronized Identifier dynamicItemId(int slot) {
        ...
        return new Identifier(MineClawd.MOD_ID, "dynamic_item_" + slotLabel(slot));
    }
    ...
    public static synchronized Identifier dynamicBlockId(int slot) {
        ...
        return new Identifier(MineClawd.MOD_ID, "dynamic_block_" + slotLabel(slot));
    }
    ...
    public static synchronized Identifier dynamicFluidBucketId(int slot) {
        ...
        return new Identifier(MineClawd.MOD_ID, "dynamic_fluid_bucket_" + slotLabel(slot));
    }
    ...
    private static void registerPlaceholders() {
        ...
        for (int slot = 0; slot < SLOT_COUNT; slot++) {
            ...
            Identifier itemId = new Identifier(MineClawd.MOD_ID, "dynamic_item_" + label);
            DynamicItem item = new DynamicItem(slotIndex, new Item.Settings());
            ITEM_PLACEHOLDERS[slotIndex] = Registry.register(Registries.ITEM, itemId, item);

            Identifier blockId = new Identifier(MineClawd.MOD_ID, "dynamic_block_" + label);
            DynamicBlock block = new DynamicBlock(slotIndex, AbstractBlock.Settings.copy(Blocks.STONE));
            BLOCK_PLACEHOLDERS[slotIndex] = Registry.register(Registries.BLOCK, blockId, block);
            DynamicBlockItem blockItem = new DynamicBlockItem(slotIndex, block, new Item.Settings());
            BLOCK_ITEM_PLACEHOLDERS[slotIndex] = Registry.register(Registries.ITEM, blockId, blockItem);

            DynamicFluid.Still still = new DynamicFluid.Still(
                    slotIndex,
                    () -> STILL_FLUIDS[slotIndex],
                    () -> FLOWING_FLUIDS[slotIndex],
                    () -> FLUID_BUCKETS[slotIndex]
            );
            DynamicFluid.Flowing flowing = new DynamicFluid.Flowing(
                    slotIndex,
                    () -> STILL_FLUIDS[slotIndex],
                    () -> FLOWING_FLUIDS[slotIndex],
                    () -> FLUID_BUCKETS[slotIndex]
            );
            Identifier stillId = new Identifier(MineClawd.MOD_ID, "dynamic_fluid_" + label);
            Identifier flowingId = new Identifier(MineClawd.MOD_ID, "flowing_dynamic_fluid_" + label);
            STILL_FLUIDS[slotIndex] = Registry.register(Registries.FLUID, stillId, still);
            FLOWING_FLUIDS[slotIndex] = Registry.register(Registries.FLUID, flowingId, flowing);

            Identifier fluidBlockId = new Identifier(MineClawd.MOD_ID, "dynamic_fluid_block_" + label);
            DynamicFluidBlock fluidBlock = new DynamicFluidBlock(
                    slotIndex,
                    still,
                    AbstractBlock.Settings.copy(Blocks.WATER).noCollision().strength(100.0F).dropsNothing()
            );
            FLUID_BLOCKS[slotIndex] = Registry.register(Registries.BLOCK, fluidBlockId, fluidBlock);

            Identifier bucketId = new Identifier(MineClawd.MOD_ID, "dynamic_fluid_bucket_" + label);
            DynamicFluidBucketItem bucket = new DynamicFluidBucketItem(
                    slotIndex,
                    still,
                    new Item.Settings().maxCount(1).recipeRemainder(Items.BUCKET)
            );
            FLUID_BUCKETS[slotIndex] = Registry.register(Registries.ITEM, bucketId, bucket);
        }

        Registry.register(
                Registries.ITEM_GROUP,
                new Identifier(MineClawd.MOD_ID, "dynamic_content"),
                FabricItemGroup.builder()
                        .displayName(Text.literal("MineClawd Dynamic"))
                        .icon(() -> new ItemStack(ITEM_PLACEHOLDERS[0]))
                        .entries((context, entries) -> addCreativeEntries(entries))
                        .build()
        );
    }
